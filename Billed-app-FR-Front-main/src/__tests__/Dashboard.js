...
/**
 * TESTS UNITAIRES SUR filteredBills
 * Périmètre de test au niveau d'une simple fonction
 */
test("filteredBills by pending status should return 1 bill", () => {
  const filtered_bills = filteredBills(bills, "pending");
  expect(filtered_bills.length).toBe(1);
});
test("filteredBills by accepted status should return 1 bill", () => {
  const filtered_bills = filteredBills(bills, "accepted");
  expect(filtered_bills.length).toBe(1);
});
test("filteredBills by accepted status should return 2 bills", () => {
  const filtered_bills = filteredBills(bills, "refused");
  expect(filtered_bills.length).toBe(2);
});

/**
 * TESTS UNITAIRES SUR DashboardUI
 * Périmètre de test au niveau du rendu simple d'une page
 */
...
/**
 * TESTS UNITAIRES Sur le périmètre logique d'une page (Rendu + Container)
 * => Rendu == UI
 * => Container == Logique métier de l'UI
 */
describe("Given I am connected as an Admin", () => {
  describe("When I am on Dashboard page and I click on arrow", () => {
    test("Then, tickets list should be unfolding, and cards should appear", async () => {
      // On court-circuite le système de routing en ne faisant qu'un rendu
      // IL n'est pas souhaité dans ces tests ici par OCR que les différents
      // "lancement" des collaborateurs externes soient fait par le routeur
      // => On veut court-circuiter l'orchestration par le routeur pour
      // pouvoir faitre notre propre orchestration dans les tests
      // (cf. app/Router.js)
      // NB: Ce "mock" aurait pu être généralisé à tous les tests
      const onNavigate = (pathname) => {
        document.body.innerHTML = ROUTES({ pathname });
      };

      // Mock du localStorage pour remplacer le vrai pas un faux qu'on peut paramétrer comme on veut
      ...
      // On initialise le container qui gère la logique métier de la page DashboardUI
